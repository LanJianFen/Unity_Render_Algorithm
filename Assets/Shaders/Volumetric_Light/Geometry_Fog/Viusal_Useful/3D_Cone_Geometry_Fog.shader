    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float getPointFalloff(float3 p)
            {
                ...
                //half radial_falloff = pow(1 - lerp(radial_ratio, radial_ratio2, _CurveShape), _HorizontalFade);
            ...
            half4 fragmentShader(v2f i) : SV_Target
            {
                ...
                _VerticalFade = 10.0h * (_VerticalFade + 0.2h); // 0.25 ，在盖子和侧面的交界线两侧，雾气的厚度是逐渐下降的，造成了交界线的雾气最厚，颜色最淡，发生了断层，所以把整个盖子直接在垂直方向去掉
                ...
                float b = 2.0f * (ray_ori_os.x * ray_dir_os.x + ray_ori_os.z * ray_dir_os.z - k2 * ray_ori_os.y * ray_dir_os.y);
                ...
                */
                ...
                // 最终融合！  
                half3 finalColor = depth_blend_falloff * average_falloff * _Intensity * _FogColor.rgb;
