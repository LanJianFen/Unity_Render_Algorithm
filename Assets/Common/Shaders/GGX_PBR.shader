Shader "Custom/GGX_PBR"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #include  "Custom_BRDF.hlsl"
            #include  "Math.hlsl"

            // 定义 MRT 输出结构体
            struct FragmentOutput
            {
                half4 specular_color : SV_Target0; // 这是最终相机的画面 (Color Buffer)
            ...
            FragmentOutput fragShader(Varings i) : SV_Target
            {
                ...
                float3 detail_normal_ts = UnpackNormal(SAMPLE_TEXTURE2D(_DetailNormalMap, sampler_DetailNormalMap, i.uv));
                ...
                // 把叠加 detail 法线后在最终法线 转移到 ws，这里用的是 final_normal_ts 右乘 tbn 的转置
                ...
                float3 specular_color = ggx_BRDF * main_light.color * NdotL * PI;

                // Diffuse
                // 根据能量守恒，高光弹飞了 F，剩下钻进物体的光就是 (1 - F)
                float3 kD = 1.0 - F;
                kD *= (1.0 - _Metallic); // 纯金属没有漫反射（光子进不去，直接吸收或弹飞）

                // Diffuse BRDF: c/PI, 但是美术认为如果给 1.0 的光不该是 1 /PI，所以给Diffuse 和 Specular 的BRDF 都乘 PI
                // C 就是 albedo.rgb
                float3 diffuse_color = kD * albedo.rgb * main_light.color * NdotL; // 普通的 PBR

                float3 Ft_in = 1.0f - F_Schlick_Custom(NdotL, F0);
                float3 sss_diffuse_irradiance = Ft_in * albedo.rgb * main_light.color * NdotL; // 专门给 SSS 打造的 diffuse 的加了菲涅尔入射率的 irradiance
                    
                FragmentOutput output;
                output.specular_color = half4(specular_color, 1.0);
