Shader "Custom/SSS/MonteCarlo_Subsurface_Scattering_Core"
{
    Properties {}
    SubShader
    {
        ...
        Pass
        {
            Tags
            {
                "LightMode" = "Subsurface Scattering MonteCarlo Core"
            }
            ...
            #include "Burley_Diffusion_Profile.hlsl"
            ...
            // 拿到我们在 C# PBR Pass 1 里画好的入流漫反射图
            TEXTURE2D(_SubsurfaceDirectDiffuseRT);
            SAMPLER(sampler_SubsurfaceDirectDiffuseRT);
            
            float4 _SubsurfaceDirectDiffuseRT_TexelSize;
            float4 _RcpSubsurfaceScatteringDistance;
            float _SurfaceScale;
            ...
            half4 frag(Varings i) : SV_Target
            {
                ...
                float3 irradiance = SAMPLE_TEXTURE2D(_SubsurfaceDirectDiffuseRT, sampler_SubsurfaceDirectDiffuseRT, i.uv).rgb;
                ...
                float base_angle = TWO_PI * GenerateHashedRandomFloat(
                    uint3(pixelCoord, (uint)(center_01_depth * 1235670.0f)));
                ...
                for (int k = 0; k < sample_count; k++)
                {
                    ...
                    SampleBurleyPDF(cdf, _RcpSubsurfaceScatteringDistance.a, sample_r, rcp_pdf);
                    ...
                    float3 sample_irradiance = SAMPLE_TEXTURE2D(_SubsurfaceDirectDiffuseRT, sampler_SubsurfaceDirectDiffuseRT, sample_uv).rgb;
                    ...
                    // 3d 世界里的真实距离
                    float3 weight = BurleyDiffusionProfile(real_sample_r, _RcpSubsurfaceScatteringDistance.rgb) * rcp_pdf;
